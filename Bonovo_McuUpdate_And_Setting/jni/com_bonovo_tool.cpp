#define LOG_TAG "com_bonovo_tool.cpp"
...
const int OTG_HOST = 1;
const int OTG_SLAVE = 2;
 ...
 * function: lowBrigthness
 *     自动音量调节
 * @parameter		- 0 自动音量功能关闭 ， 非零 音量比例值，从10到100，表示原背光值的10%到100%
 * @return           返回执行结果
 *            - 0     执行成功
 *            - -1	  串口没有打开
 *************************************************************************************
 ...
 * function: pickColor
 *     閼冲本娅欐０婊嗗鐠嬪啳濡�
 * @parameter		- red 缁俱垼澹婇崐锟�- green 缂佽儻澹婇崐锟� blue 閽冩繆澹婇崐锟� * @return           鏉╂柨娲栭幍褑顢戠紒鎾寸亯
 *            - 0     閹笛嗩攽閹存劕濮�
 *            - -1	  娑撴彃褰涘▽鈩冩箒閹垫挸绱�
 *************************************************************************************
 */
static int android_mcu_pickColor(JNIEnv *env, jobject thiz, jint red, jint green, jint blue)
{
	int writeSize,fd;
	unsigned int sum = 0;
	unsigned char cmdBuf[11] = {0xFA, 0xFA, 0x0B, 0x00, 0x83, 0x0A};
	LOGE("pickColor() into!!!");
 ...
 * function: switchOTG
 *     切换OTG/连接电脑模式
 * @parameter		- model 选择的模式
 * @return           鏉╂柨娲栭幍褑顢戠紒鎾寸亯
 *            - 0     閹笛嗩攽閹存劕濮�
 *            - -1	  娑撴彃褰涘▽鈩冩箒閹垫挸绱�
 *************************************************************************************
 */
static int android_mcu_switchOTG(JNIEnv *env, jobject thiz, jint model)
{
	int fd,state = -1;
	fd = open(MCU_CTRL_NODE, O_RDWR|O_NOCTTY|O_NONBLOCK);
			if (-1 == fd)
			...
			else if(model == OTG_HOST)
			{
				LOGE("open bonovo_handle secess, fd = 0x%x\n",fd);
				state = ioctl(fd, IOCTL_HANDLE_OTG_HOST);
				close(fd);
			}else if(model == OTG_SLAVE){
				LOGE("open bonovo_handle secess, fd = 0x%x\n",fd);
				state = ioctl(fd, IOCTL_HANDLE_OTG_SLAVE);
				close(fd);
			}
		return state;
}
 ...
 * function: setStandby
 *     设置待机时间
 * @parameter		- times 待机时间(该值乘10表示休眠的分钟)
 * @return           鏉╂柨娲栭幍褑顢戠紒鎾寸亯
 *            - 0     閹笛嗩攽閹存劕濮�
 *            - -1	  娑撴彃褰涘▽鈩冩箒閹垫挸绱�
 *************************************************************************************
 */
static int android_mcu_setStandby(JNIEnv *env, jobject thiz, jint times)
{
	int writeSize,fd,conversionTimes;
		...
		if(times == 65535){
			...
			conversionTimes = times / 10; //参数转换
		}

		cmdBuf[6] = conversionTimes & 0XFF;
		cmdBuf[7] = (conversionTimes>>8) & 0XFF;
		sum = checkSum(cmdBuf, cmdBuf[2]-2);
		cmdBuf[8] = sum & 0xFF;
		cmdBuf[9] = (sum>>8)& 0xFF;

		if((fd = open(RADIO_DEV_NODE, O_RDWR|O_NOCTTY|O_NONBLOCK)) < 0) {
				LOGI("======bonovo dev/ttyS3 setStandby open result = %d\n",fd);
				return -1;
			} else {
				writeSize = write(fd, cmdBuf, cmdBuf[2]);
				LOGE("setStandby writeSize = %d ",writeSize);
				close(fd);
				return 0;
			}
}

static const char *classPathName = "com/bonovo/mcuupdate_and_setting/FragmentService";
static JNINativeMethod methods[] = {
   	...
   	{"jniswitchOTG", "(I)I", (void *)android_mcu_switchOTG},
   	{"jniSetStandby", "(I)I", (void *)android_mcu_setStandby},
};
